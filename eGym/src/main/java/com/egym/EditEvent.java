...
public class EditEvent extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int urlEventID = Integer.parseInt(path.substring(1));
            EventsModel nm = new EventsModel();
            request.setAttribute("Event", nm.getSingleEvent(urlEventID));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
         Class.forName("com.mysql.jdbc.Driver").newInstance();
            con = DriverManager.getConnection(url, user, password);
            //`(IN pIdActivities int(11), pTitle varchar(100), pPoints int(11), pStartTime datetime, pEndTime datetime, pUsers_Username varchar(30), pBody text, pActivityTypes_idActivityTypes int(11))
            CallableStatement cs = this.con.prepareCall("{call edit_activity(?,?,?,?,?,?,?,?)}");
            cs.setInt(1, urlEventID);
            cs.setString(2, title);
            cs.setInt(3, points);
            cs.setTimestamp(4, startDateTime);
            cs.setTimestamp(5, endDateTime);
            cs.setString(6, trainer);
            cs.setString(7, body);
            cs.setInt(8, type);
            cs.executeQuery();
            cs.close();
